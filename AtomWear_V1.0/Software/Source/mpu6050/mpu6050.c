...
void mpu6050_init()
{
    ...
    send_rec[0] = 0x00;
    mpu6050_write(MPU6050_ADDR0, PWR_MGMT_1, send_rec, 1);  // 关闭睡眠状态, 开温度
                                 // 0x6B
    send_rec[0] = 0x07;
    mpu6050_write(MPU6050_ADDR0, SMPRT_DIV, send_rec, 1);  // 一秒1000个采样率
                                 // 0x19
    // 低通滤波，  
    send_rec[0] = 0x06;
    mpu6050_write(MPU6050_ADDR0, CONFIG, send_rec, 1);  //  
                                 // 0x1A
    send_rec[0] = 0x18;
    mpu6050_write(MPU6050_ADDR0, GYRO_CFG, send_rec, 1);  // 000 11 000  陀螺仪量程：1000度/s
                                 // 0x1B
    send_rec[0] = 0x08;
    mpu6050_write(MPU6050_ADDR0, ACCEL_CFG, send_rec, 1);  // 000 11 000  加速度量程：4g
                                 // 0x1C
    // 缓冲区，默认关闭
    // 中断使能，默认关闭
