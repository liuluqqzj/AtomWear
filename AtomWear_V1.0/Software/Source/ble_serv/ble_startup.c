...
typedef struct
{
    uint8_t data[BLE_SEND_SIZE];   // 发送缓冲区
    uint8_t length;     // 发送字节数， 未满20时，其值指向下一个未存储字节区
    uint32_t offset;    // 读取偏移量，指向下一个要读取的字节处
...
void ble_rec_init()
{
    ...
    ble_rec.front  =  0;    // 初始化指向未存储的地址
    ble_rec.rear   =  0;    // 初始化指向第一个有效数据处
    ble_rec.count  =  0;
...
static void timers_init(void)
{
    ...
    // 定时器初始化       RTC分频：0      计数器的最大个数：2   定时器操作队列大小：4   不使用事件调度
