...
void timer1_stop()
{
    ...
    NRF_TIMER1->INTENSET       = TIMER_INTENSET_COMPARE0_Disabled << TIMER_INTENSET_COMPARE0_Pos;   // ¿ªÍ¨ÑÓÊ±ÖÐ¶Ï1
    //NRF_TIMER1->INTENSET       = TIMER_INTENSET_COMPARE1_Disabled << TIMER_INTENSET_COMPARE0_Pos;   // ¿ªÍ¨ÑÓÊ±ÖÐ¶Ï1
}
...
void pedometer_startup()
{
    ...
    else
    {
        ...
        if(cur.value >= pre.value)
        {
            ...
            else  if( (cur.value < Thr_Down) || ( (peak[spe_num].value - valley[spe_num].value) > Thr_Wide) )    // µÍÓÚµÍãÐÖµÂð£¿»òÕß·ù¶ÈÂú×ã
            {
