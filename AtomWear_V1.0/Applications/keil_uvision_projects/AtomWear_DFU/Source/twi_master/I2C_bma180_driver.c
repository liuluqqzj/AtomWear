...
/*void GPIOTE_IRQHandler()
{
	...
	xprintf("bma interrupt happened!\n");
	signal = 1;
    }
}
...
{
  nrf_gpio_cfg_input(nRF_INT_pin, NRF_GPIO_PIN_NOPULL);
	// Enable interrupt:
  NVIC_EnableIRQ(GPIOTE_IRQn);
  NRF_GPIOTE->CONFIG[0] =  (GPIOTE_CONFIG_POLARITY_LoToHi << GPIOTE_CONFIG_POLARITY_Pos)
                           | (nRF_INT_pin << GPIOTE_CONFIG_PSEL_Pos)  
...
void bma_new_data_config()
{
    bma_write(0x21, 0x03);
	nrf_delay_ms(10);
}
	
void bma_high_g_config()
{
    //uint8_t send_rec;
...
void bma_high_g_close()
{
    bma_write(0x21, 0x08);
}


void bma180_tap_config()
{
	uint8_t send_rec;
...
void bma_new_data_test()
{
	IIC_Init();
	bma180_basic_config();
	bma_new_data_config();
	nRF_gpio_INT_config(ACCE_TAP_PIN_NUMBER);	//开引脚中断
...
extern void system_high_g_dect_open()
{
	bma_high_g_config();
}
    
extern void system_high_g_dect_close()
{
	bma_high_g_close();
}

extern void system_gsensor_init()
{
    IIC_Init();
    bma180_basic_config();
    bma180_tap_config();  
