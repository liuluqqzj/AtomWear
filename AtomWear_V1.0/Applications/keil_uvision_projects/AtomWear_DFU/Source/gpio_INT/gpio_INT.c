...
{
    if((NRF_GPIOTE->EVENTS_IN[0] == 1) && (NRF_GPIOTE->INTENSET & GPIOTE_INTENSET_IN0_Msk) ) //上升沿
    {
    ...
    if((NRF_GPIOTE->EVENTS_IN[1] == 1) && (NRF_GPIOTE->INTENSET & GPIOTE_INTENSET_IN1_Msk)  ) //上升沿
    {
...
{
    if((NRF_TIMER1->EVENTS_COMPARE[0] == 1) && (NRF_TIMER1->INTENSET & TIMER_INTENSET_COMPARE0_Msk))     // 仅用来去抖延时
    {
        NRF_TIMER1->EVENTS_COMPARE[0] = 0;
        NRF_TIMER1->TASKS_CLEAR = 1;
        if(SYSTEM_WORK_MODE != CURRENT_WORK_MODE) //按键处理
        {
            switch(SYSTEM_WORK_MODE)
            {
            case SYSTEM_PEDOMETER_MODE:
...
{
    if((NRF_TIMER2->EVENTS_COMPARE[0] == 1) && (NRF_TIMER2->INTENSET & TIMER_INTENSET_COMPARE0_Msk))     // 仅用来去抖延时
    {
        NRF_TIMER2->EVENTS_COMPARE[0] = 0;
        NRF_TIMER2->INTENSET = TIMER_INTENSET_COMPARE0_Disabled << TIMER_INTENSET_COMPARE0_Pos; // 关定时器延时中断0

        NRF_TIMER2->TASKS_STOP = 1;
        xprintf("It is changed..\r\n");
    }

    if((NRF_TIMER2->EVENTS_COMPARE[1] == 1) && (NRF_TIMER2->INTENSET & TIMER_INTENSET_COMPARE1_Msk))     // 仅用来去抖延时
    {
        ...
        {
            NRF_TIMER2->INTENSET = TIMER_INTENSET_COMPARE1_Disabled << TIMER_INTENSET_COMPARE1_Pos; // 关定时器延时中断0
            NRF_TIMER2->TASKS_STOP = 1;
        }
    }
}
